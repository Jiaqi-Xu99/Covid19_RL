...
class CovidEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.p_test_nosym = 0.25 # Probablity that a person get covid test if no symptom presents
        ...
        """
        self.observation_space = spaces.Box(low=-1, high=1, shape=(self.size * self.days,), dtype=np.float32)
        self.current_state = np.full((self.size * self.days,), -1, dtype=np.float32)
        self.observed_day = 0
        for i in range(self.size):
            self.current_state[i * self.days + self.observed_day] = self.simulated_state["Prediction"][i][self.observed_day]
    ...
    def step(self, action):
        ...
        sum1 = 0
        sum2 = 0
        sum3 = 0 # total number of covide testing
        ...
        for i in range(self.size):
            ...
            if self.simulated_state["Whether infected"][i][self.observed_day] == 0 and quarantine[i] == 1:
                sum2 = sum2 + 1
            if self.simulated_state["Test results"][i][self.observed_day] != -1:
                sum3 = sum3 + 1
        ...
        reward = -1 * sum1 - self.ratio * sum2 #TODO minus the cost for conducting covid testing
        self.observed_day = self.observed_day + 1
    ...
    def _simulation(self):
        ...
        self.simulated_state = {
            ...
            "Test results": np.full((self.size, self.days), -1) #-1 for no test, 0 for negative result, 1 for positive result
            }
        ...
        for i in range(self.size):
            ...
            for i in range(self.size):
                for j in range(self.days):
                    # if case shows sympton
                    if(self.simulated_state["Showing symptoms"][i][j] == 1):
                        self.simulated_state["Test results"][i][j] = self.simulated_state["Whether infected"][i][j]
                    # if case shows no sympton
                    else:
                        # Use log normal distribution, mean = p_test_nosym, std = 0.5
                        p_test = int(np.random.lognormal(self.p_test_nosym, 0.5, 1))
                        if p_test > 0.5:
                            self.simulated_state["Test results"][i][j] = self.simulated_state["Whether infected"][i][j]  
